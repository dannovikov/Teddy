...
_planner = Agent(
    ...
    "Lastly, only when the user's task is completely fulfilled, RUN PYTEST one more time, and if it passes, issue the termination token 'TASK_COMPLETE'. ",
...
_coder = Agent(
    ...
    "If there is a pending request for code to be written, it is your job to write it in a file using the write_file function. "
    "No raw code, always use write_file to write code to the disk. To edit a file, read it using read_file, then write the modified content with write_file. ",
    tools=unix_tools,
    disallow_transfer_to_peers=True,
    disallow_transfer_to_parent=True,
)
...
loopbreaker = Agent(
    model=LiteLlm(model="openai/gpt-4.1-nano"),
    name="loopbreaker",
    description="You are a loopbreaker agent responsible for breaking the loop of agents that are stuck in a cycle. You are part of a larger cycle of agents [planner, specifier, coder, tester, reviewer]. Your job is to identify when the agents are stuck and issue a new task to break the loop.",
    instruction="Be brief. You are a loopbreaker agent responsible for breaking the loop of agents that are stuck in a cycle. Your job is to identify when the agents are stuck and let everyone know really loudly. "
    "If you detect that the agents are not making progress, passing the batton, saying they will do thing but not, then say in all caps, 'GUYS, YOU ARE STUCK IN A LOOP. PLANNER, ISSUE A NEW TASK TO A SPECIFIC AGENT.' "
    "Do not write or execute code yourself; focus solely on breaking the loop. If there is no loop, say `OK` and nothing else."
    "Don't let the agents ask eachother to run specific files or commands repeatedly. If the coder is speaking but not coding, say 'Coder, stop talking and code.' ",
    disallow_transfer_to_peers=True,
    disallow_transfer_to_parent=True,
)
...
system = LoopAgent(
    ...
    max_iterations=20,
    sub_agents=[_planner, _specifier, _coder, _tester, _reviewer, loopbreaker],
)
